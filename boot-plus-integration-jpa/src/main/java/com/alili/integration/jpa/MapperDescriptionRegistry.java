 ...
 * @since 1.0
 */
public class MapperDescriptionRegistry {

    private final Map<Type, MapperDefinition> entityMapperRelation = new ConcurrentHashMap<>();

    public MapperDescriptionRegistry() {
    }

    public Class register(Class<?> mapperClass) {
        synchronized (entityMapperRelation) {
            MapperDefinition mapperDesc = new MapperDefinition(mapperClass);
            Type entityType = mapperDesc.getGenericType().getDomainType();
            if (entityType instanceof Class) {
                Class type = (Class) entityType;
                if (!type.isAnonymousClass() && !type.isInterface() && !type.isMemberClass()) {
                    entityMapperRelation.put(entityType, mapperDesc);
                }
    ...
    public MapperDefinition getMapperDescription(Type entityType) {
