...
import com.alili.integration.jpa.JpaInitializer;
import com.alili.integration.spring.DatabaseRegistration;
import com.alili.integration.spring.DatabaseRegistry;
import com.alili.integration.spring.MybatisJpaConfigurer;
...
public class MybatisJpaStart implements ApplicationListener<ContextRefreshedEvent>, ApplicationContextAware {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        if(!CollectionUtils.isEmpty(mybatisJpaConfigurers)) {
            mybatisJpaConfigurers.forEach(mybatisJpaConfigurer -> {
                mybatisJpaConfigurer.addDatabase(databaseRegistry);
            });
        }
        ...
        databaseRegistrations.forEach(databaseRegistration -> {
            ...
            if(databaseRegistration.getPaginationDialect() == null) {
                logger.warn("DatabaseType: {} does not provider pagination dialect, see class com.alili.integration.dialect.pagination.PaginationDialect", databaseRegistration.getDatabaseType().getDatabaseId());
            } else {
