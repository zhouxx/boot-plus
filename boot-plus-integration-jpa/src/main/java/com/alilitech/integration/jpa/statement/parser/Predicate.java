...
public class Predicate {
    ...
    public Predicate(String predicate, Class<?> domainClass, MethodDefinition methodDefinition) {
        ...
        this.orderBySource = parts.length == 2 ? new OrderBySource(parts[1], Optional.of(domainClass)) : null;
    ...
    private class OrPart implements Iterable<Part> {
        ...
        OrPart(String source, Class<?> domainClass) {
            ...
            for (String part : split) {
                if (StringUtils.hasText(part)) {
                    children.add(new Part(part, domainClass, methodDefinition));
                }
            }
